...
#in src.model.encoder HGFilters.HGFilter, or pointnet.LocalPoolPointnet
...
def get_model(cfg, device=None):
    ...
    # encoder(s)
    encoder_str = cfg_model['encoder'] #pointnet_local_pool
    encoder_kwargs = cfg_model['encoder_kwargs']#details about the model
    encoder = encoder_dict[encoder_str](dim=dim, **encoder_kwargs)
    #in encoder/__init__.py, 
    # "**" means it takes in a dictionary that contains a lot of parameters as input of model
    
    # decoder
    decoder_str = cfg_model['decoder']#simple_local_multi_class
    ...
    if 'conv_onet' == cfg_model['method']:
        ...
        #.get can handle missing keys or Fallback Value, just use defalt
