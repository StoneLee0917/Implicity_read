...
class LocalPoolPointnet(nn.Module):
    ...
    def __init__(self, feature_dim=128, dim=3, hidden_dim=128, scatter_type='max',
        ...
        #Applies a linear transformation to the incoming data y=A^T x + b
        #a fully connected layer that takes input of dimension dim and produces output of dimension 2 * hidden_dim.        
        self.blocks = nn.ModuleList([
            ResnetBlockFC(2 * hidden_dim, hidden_dim) for i in range(n_blocks)
        ])
        #nn.ModuleList: Holds submodules in a list.
        #nn.Module: Base class for all neural network modules
        #a stack of Residual Blocks, specifically ResnetBlockFC, repeated n_blocks times.
    ...
    def forward(self, inputs: torch.Tensor, **kwargs):
        ...
        Args:
            ...
            b: batch_size, point cloud number in the batch, 
            n: point number, 3: dimension
            **kwargs:
        ...
        #applies the first block from the list of blocks (self.blocks[0]) to the net tensor. 
        for block in self.blocks[1:]:
        #This loop iterates over the remaining blocks in self.blocks, skipping the first block 
