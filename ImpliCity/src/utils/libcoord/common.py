...
def coordinate2index(x, reso, coord_type='2d'):
    ...
    #x= inputs.clone()[:, :, [0, 1]]
    x = (x * reso).long()
    """    scales the normalized coordinates by reso and converts them 
    to integers by applying .long(). This operation maps the 
    normalized coordinates to the grid size."""
    if coord_type == '2d':  # plane
        index = x[:, :, 0] + reso * x[:, :, 1]  # [B, N, 1]
        #x[:, :, 0] extracts all elements along the first axis (batch dimension), all elements along the second axis (number of points dimension), and only the elements along the third axis (x-coordinate). 
        #The formula computes the grid index by multiplying the y-coordinates by reso and adding them to the x-coordinates.
    index = index[:, None, :]  # [B, 1, N]reshapes the index tensor. It adds an extra dimension to the tensor to make it compatible with other tensors. 
    return index
